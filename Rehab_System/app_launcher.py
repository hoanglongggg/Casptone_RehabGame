...
class GameSelection(QWidget):
    ...
    def start_game(self, game_name):
        ...
        # `game_name` trong code cũ là "tên hiển thị". Ta cần key tương ứng.
        # Suy ra key bằng cách map lại theo label.
        label_to_key = {
            "🍎 Game Hái Táo": "apple_game",
            "🏎️ Game Đua Xe": "racing_game",
            "💪 Bài tập Co Duỗi": "flex_exercise",
        }
        game_key = label_to_key.get(game_name, "flex_exercise")
        self._active_game_key = game_key

        # Disable các game khác để tránh mở nhiều camera/subprocess.
        ...
        self.bridge_proc = start_bridge_subprocess()

        # 2) QThread camera chạy để không treo UI
        self.camera_worker = CameraWorker(patient_id=self.patient_id, game_name=game_key, max_duration_sec=60)
    ...
    def on_stats_update(self, left_reps: int, right_reps: int, elapsed_sec: float) -> None:
        self.lbl_status.setText(
            f"Đang tập... Reps(L/R): {left_reps}/{right_reps} | Thời gian: {format_mm_ss(elapsed_sec)}"
        )
    ...
    def on_camera_error(self, msg: str) -> None:
        stop_bridge_subprocess(self.bridge_proc)
